...
class StockEntry(StockController):
    ...
    def check_if_operations_completed(self):
        ...
        for d in prod_order.get("operations"):
            ...
            # BAZZ - operation is completed when its state is 'Completed'
            if d.status != 'Completed':
                frappe.throw(_("Row #{0}: Operation {0} is not completed for {1}").format(d.operation, self.production_order))
    ...
    def set_basic_rate(self, force=False, update_finished_item_rate=True):
        ...
        for d in self.get('items'):
            ...
            # get basic rate
            if not d.bom_no:
                if not flt(d.basic_rate) or force:
