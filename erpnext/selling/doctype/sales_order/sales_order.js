			...
			if(doc.status != 'Closed') {
				...
				if (this.frm.has_perm("submit")) {
					...
					if(flt(doc.per_delivered, 2) < 100 || flt(doc.per_billed) < 100) {
						this.frm.add_custom_button(__('Close'), function() {
						    me.close_sales_order() }, __("Status"))
					}
				}
				...
				if(flt(doc.per_delivered, 2) < 100 && ["Sales", "Shopping Cart"].indexOf(doc.order_type)!==-1 && allow_delivery) {
					this.frm.add_custom_button(__('Delivery'),
						function() { me.make_delivery_note_based_on_delivery_note(); });
					this.frm.add_custom_button(__('Production Order'),
						function() { me.make_production_order() });
				...
				if(!doc.order_type || ["Sales", "Shopping Cart"].indexOf(doc.order_type)!==-1
					...
					this.frm.add_custom_button(__('Material Request'),
						function() { me.make_material_request() });
				}
				...
				if(flt(doc.per_billed)==0) {
					this.frm.add_custom_button(__('Payment Request'),
						function() { me.make_payment_request() });
					this.frm.add_custom_button(__('Payment'),
						function() { me.make_payment_entry() });
				}
				...
				if(flt(doc.per_delivered, 2) < 100 &&
					...
					this.frm.add_custom_button(__('Maintenance Visit'),
						function() { me.make_maintenance_visit() });
