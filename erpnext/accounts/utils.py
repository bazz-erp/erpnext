...
def get_stock_rbnb_difference(posting_date, company):
    ...
    pr_valuation_amount = frappe.db.sql("""
		...
		and pr.posting_date <= %s and pr_item.item_code in (%s)""" %
                                        ...
                                        tuple([company, posting_date] + stock_items))[0][0]
...
# get the balance of the last daily movement for specific payment_mode considering the date given
def get_last_daily_movement_balance(date, payment_mode):
    result = frappe.db.sql(
        """select income, expenditures from `tabDaily Movement` where 
        target_date < %(target_date)s and payment_mode=%(payment_mode)s order by target_date desc limit 1""",
        {"target_date": date, "payment_mode": payment_mode}, as_dict=1)
